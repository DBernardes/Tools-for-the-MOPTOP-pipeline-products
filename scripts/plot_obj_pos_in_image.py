...
min, max =  59775, 60051
experiment = 'all data'
src_path = os.path.join('..', '..', 'Pol charact MOPTOP', 'Low polarized stars', star_name, experiment, 'selected_files', f'{min}-{max}')
...
for camera in [3, 4]:
    for idx, filter in enumerate(['V', 'R', 'I']):
        ax = axs[camera-3, idx]
        new_path = os.path.join(src_path, f'cam{camera}', f'{filter} Filter')
