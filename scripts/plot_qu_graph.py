...
calculated_qu = pd.read_csv(csv_file_name)
...
#             qu_row = calculated_qu.loc[calculated_qu["filter"] == filter]
#             inst_q, inst_std_q, inst_u, inst_std_u = (
#                 qu_row["q"].values,
#                 qu_row["std_q"].values,
#                 qu_row["u"].values,
#                 qu_row["std_u"].values,
...
fig, axs = plt.subplots(
    len_stars, 5, sharey="row", sharex="row", figsize=(5 * len_stars, 10)
)
for row_idx, star in enumerate(stars):
    row_axs = axs[row_idx]
    colors = ["b", "r", "g", "k", "m"]
    csv_file_name = os.path.join(
        base_path, "High polarized stars", star, "reduced", star, "manipulated_data.csv"
    )
