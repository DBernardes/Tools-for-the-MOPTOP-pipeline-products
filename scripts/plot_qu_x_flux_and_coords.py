...
fig, axs = plt.subplots(3, 4, figsize=(15, 7), sharey='row')
axs[2,0].set_xlabel('Photons cam1')
axs[2,1].set_xlabel('Photons cam2')
axs[2,2].set_xlabel('X axis')
axs[2,3].set_xlabel('Y axis')
camera = 4
alpha = 0.7
fontsize = 9
...
for idx, filter in enumerate(['V', 'R', 'I']):
    ...
    rows = rows.sort_values(by=[f's1_src_cam{camera-3}'])
    ax = axs[idx, 0]
    photons, q, u, res_p1q, res_p1u = calc_plot_parameters(rows, f's1_src_cam{camera-3}')
    plot_graph(ax, photons, q, u, res_p1q, res_p1u, filter, tmp)


    ax = axs[idx, 1]
    rows = rows.sort_values(by=[f's1_src_cam{camera-2}'])
    photons, q, u, res_p2q, res_p2u = calc_plot_parameters(rows, f's1_src_cam{camera-2}')
    plot_graph(ax, photons, q, u, res_p2q, res_p2u, filter, tmp)

    #------------------------------------------------------------

    new_path  = os.path.join(base_path, star_name, 'selected_files', f'{min}-{max}', f'cam{camera}', f'{filter} Filter')
    xcoord, ycoord = get_coords_in_series(new_path, rows['date'])
    rows['xcoord'] = xcoord
    rows['ycoord'] = ycoord
