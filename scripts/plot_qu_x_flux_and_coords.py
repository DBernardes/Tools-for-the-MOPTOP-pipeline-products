...
fig, axs = plt.subplots(1, 3, figsize=(15, 7), sharey="row")
axs[0].set_xlabel(f"Photons cam{camera}")
...
def plot_graph(ax, x, q, q_err, u, u_err, res_q, res_u, filter, tmp):
    ...
    ax.annotate(
        f"coef_q={res_q.statistic:.3f}, pval_q={res_q.pvalue:.2e}\ncoef_u={res_u.statistic:.3f}, pval_u={res_u.pvalue:.3f}",
        xy=(0.05, 0.95),
        xycoords="axes fraction",
        ha="left",
        va="top",
        fontsize=fontsize,
        bbox=dict(boxstyle="round", fc="lightgray", ec="black", alpha=0.5),
    )
...
for idx, filter in enumerate(["V"]):
    ...
    ax = axs[0]
    photons, q, q_err, u, u_err, res_p1q, res_p1u = calc_plot_parameters(
        rows, f"s1_src_cam{camera-2}"
    )
    ...
    new_path = os.path.join(
        base_path, star_name
    )  # , 'selected_files', f'{min}-{max}', f'cam{camera}', f'{filter} Filter')
    xcoord, ycoord = get_coords_in_series(
        new_path, rows["date"], rows["mjd"], f"{camera}_e"
    )
    rows["xcoord"] = xcoord
    rows["ycoord"] = ycoord
