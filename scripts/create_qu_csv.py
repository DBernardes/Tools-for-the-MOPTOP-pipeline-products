...
for star in low_polarized_stars.keys():
    base_path = os.path.join('..', '..', 'Pol charact MOPTOP', 'Low polarized stars', star, 'all data', 'reduced', star )
    ...
    for filter in ['B', 'V', 'R', 'I', 'L']:
        ...
        if filter == 'B' and star != 'BD+32 3739': #this because we do not have enough data for the filter B for the other stars
            continue
        if rows.shape[0] == 0:
            continue
        q = rows['q_avg']
