...
obj_coords_file = "./tests/useful_files/coords_over_series.csv"
...
dir_path = os.path.join(dir_path, "..", "shifted_images")


def test_get_moptop_file_name(ffile_man):
    ffile = ffile_man.cam_files["cam3"][0]
    file_name = os.path.join(dir_path, ffile.name)
    hdr = fits.getheader(file_name)
    assert ffile_man._get_moptop_file_name(hdr) == hdr["EXPID"][:-1] + "1.fits"
...
def test_combine_images_by_run(ffile_man):
    ...
    run_numbers = [obj.run_num for obj in ffile_man.cam_files["cam3"]]
